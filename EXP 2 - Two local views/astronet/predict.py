...
def _process_tce(feature_config):
  ...
  Raises:
    ...
    'local_view', and 'local_view_2'.
  """
  if not {"global_view", "local_view", "local_view_2"}.issuperset(feature_config.keys()):
    raise ValueError(
        "Only 'global_view', 'local_view', and 'local_view_2' features are supported.")

  # Read and process the light curve.
  time, flux = preprocess.read_light_curve(FLAGS.kepler_id,FLAGS.kepler_data_dir)
  time, flux = preprocess.process_light_curve(time, flux)
  ...
  if "local_view_2" in feature_config:
    local_view_2 = preprocess.local_view_2(time, flux, FLAGS.period, FLAGS.duration)
    # Add a batch dimension.
