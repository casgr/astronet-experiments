...
def _process_tce(feature_config):
  ...
  Raises:
    ...
    and 'local_view'.
  """
  if not {"global_view_1", "global_view_2", "local_view_1", "local_view_2"}.issuperset(feature_config.keys()):
    raise ValueError(
        "Only 'global_view_1', 'global_view_2', 'local_view_1', and 'local_view_2' features are supported.")

  # Read and process the light curve.
  time_1, flux_1, time_2, flux_2 = preprocess.read_light_curve(FLAGS.kepler_id,FLAGS.kepler_data_dir)
  time_1, flux_1, time_2, flux_2 = preprocess.process_light_curve(time_1, flux_1, time_2, flux_2)
  time_1, flux_1 = preprocess.phase_fold_and_sort_light_curve(
      time_1, flux_1, FLAGS.period, FLAGS.t0)
  time_2, flux_2 = preprocess.phase_fold_and_sort_light_curve(
      time_2, flux_2, FLAGS.period, FLAGS.t0)

  # Generate the local and global views.
  features = {}

  if "global_view_1" in feature_config:
    global_view = preprocess.global_view_1(time_1, flux_1, FLAGS.period)
    # Add a batch dimension.
    features["global_view_1"] = np.expand_dims(global_view_1, 0)
    
  if "global_view_2" in feature_config:
    global_view = preprocess.global_view_2(time_2, flux_2, FLAGS.period)
    # Add a batch dimension.
    features["global_view_2"] = np.expand_dims(global_view_2, 0)

  if "local_view_1" in feature_config:
    local_view = preprocess.local_view_1(time_1, flux_1, FLAGS.period, FLAGS.duration)
    # Add a batch dimension.
    features["local_view_1"] = np.expand_dims(local_view_1, 0)
    
  if "local_view_2" in feature_config:
    local_view_2 = preprocess.local_view_2(time_2, flux_2, FLAGS.period, FLAGS.duration)
    # Add a batch dimension.
    features["local_view_2"] = np.expand_dims(local_view_2, 0)
