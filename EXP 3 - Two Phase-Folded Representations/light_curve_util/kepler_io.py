...
def read_kepler_light_curve(filenames,
  ...
  Returns:
    all_time_1: A list of numpy arrays; the time values of the light curve (first half of available observations).
    all_flux_1: A list of numpy arrays corresponding to the time arrays in
        all_time (first half of available observations).
    all_time_2: A list of numpy arrays; the time values of the light curve (second half of available observations).
    all_flux_2: A list of numpy arrays corresponding to the time arrays in
        all_time (second half of available observations).
  """
  all_time = []
  all_flux = []
  all_time_1 = []
  ...
  for filename in filenames:
    ...
    ltime = np.int(len(all_time)/2)
    ltimelen = np.int(len(all_time))
    lflux = np.int(len(all_flux)/2)
    lfluxlen = np.int(len(all_flux))
    all_time_1 = all_time[0:(ltime+1)]
    all_time_2 = all_time[ltime:ltimelen]
    all_flux_1 = all_flux[0:(lflux+1)]
    all_flux_2 = all_flux[lflux:lfluxlen]
  

  return all_time_1, all_flux_1, all_time_2, all_flux_2
